"""CLI commands for finn.no 'Bolig til salgs' (homes for sale)."""
...
from ..marketplaces.realestate import (
    OWNERSHIP_TYPES,
    PROPERTY_TYPES,
    SORT_MAP,
    RealestateClient,
...
app = typer.Typer(help="finn.no Eiendom — bolig til salgs (homes for sale).")
console = Console()
err = Console(stderr=True)

SORT_HELP = (
    "Sort order. Options: newest (default), oldest, "
    "price-asc, price-desc, area-asc, area-desc."
)
TYPE_HELP = (
...
OWNERSHIP_HELP = (
    "Ownership form (repeatable). Options: "
    + ", ".join(sorted(set(OWNERSHIP_TYPES)))
    + "."
)


def _price_str(value: int | None) -> str:
    if value is None:
        return "–"
...
def search(
    ...
    price_min: Annotated[int | None, typer.Option("--price-min", help="Minimum asking price (NOK)")] = None,
    price_max: Annotated[int | None, typer.Option("--price-max", help="Maximum asking price (NOK)")] = None,
    ...
    ownership: Annotated[
        list[str] | None,
        typer.Option("--ownership", "-o", help=OWNERSHIP_HELP),
    ] = None,
    ...
    """Search homes for sale on finn.no."""
    ...
    try:
        ...
        if ownership:
            for o in ownership:
                resolve_ownership_type(o)
    except ValueError as e:
        err.print(f"[red]Error:[/red] {e}")
        raise typer.Exit(1)

    with FinnClient() as finn:
        client = RealestateClient(finn)
        try:
    ...
    if plain:
        for ad in ads:
            print(
                f"{_price_str(ad.price):<14} "
                f"{(ad.property_type or '–'):<14} "
    ...
    if ownership:
        filters.append("/".join(ownership))
    if bedrooms_min is not None:
        filters.append(f"≥{bedrooms_min} sov")
    if price_min is not None:
        filters.append(f"fra {_price_str(price_min)}")
    if price_max is not None:
        filters.append(f"til {_price_str(price_max)}")
    if location:
    ...
    console.print(
        f"\n[bold]Bolig til salgs[/bold]{filter_str} — "
    ...
    table.add_column("Pris", justify="right", width=13, no_wrap=True)
    table.add_column("Type", width=13, no_wrap=True, overflow="ellipsis")
